 ...
 */
public class Test06 {
    private static Logger logger = LogManager.getLogger(Test06.class);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // 监听队列
            channel.basicConsume(ConnectionUtil.QUEUE_SEND_NAME6, true, consumer);
            // 获取消息
            QueueingConsumer.Delivery delivery = consumer.nextDelivery();
            String message2 = new String(delivery.getBody());
            logger.info("接收的消息，通道是：" + ConnectionUtil.QUEUE_SEND_NAME6 + " Received：" + message2);
...
//            JobStep jobStep1 = ConnectionUtil.getJobStep(QUEUE_SEND_NAME6, sqlforname, sqladdress, sqlusername, sqlpassword);
...
//            JobStep jobStep1 = ConnectionUtil.getJobStep(QUEUE_SEND_NAME6, sqlforname, sqladdress, sqlusername, sqlpassword);
            ...
            channel.basicPublish("", ConnectionUtil.QUEUE_REV_NAME6, null, object1.toJSONString().getBytes());

            logger.info("发送的消息，通道是：" + ConnectionUtil.QUEUE_REV_NAME6 + " sends：" + object1.toJSONString());
        } catch (Exception e) {
            try {
                JobStep jobStep = new JobStep();
                jobStep.setJobname(ConnectionUtil.QUEUE_SEND_NAME6);
