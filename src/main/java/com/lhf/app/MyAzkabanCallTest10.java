 ...
 * @Description:
 */
public class MyAzkabanCallTest10 {
    private static Logger logger = LogManager.getLogger(MyAzkabanCallTest10.class);
    ...
    public static void main(String[] args) throws Exception {
        ...
        jobStep.setJobname(ConnectionUtil.QUEUE_SEND_NAME10);
        ...
        JobStep jobStep1 = ConnectionUtil.getJobStep(ConnectionUtil.QUEUE_SEND_NAME10, ConnectionUtil.sqlforname, ConnectionUtil.sqladdress, ConnectionUtil.sqlusername, ConnectionUtil.sqlpassword);
        if (jobStep1.getJobstatus() != null && jobStep1.getJobstatus().equals("2")) {
            logger.error("===============FAILURE==============");
            logger.error("任务已存在：" + ConnectionUtil.QUEUE_SEND_NAME10);
            throw new Exception("任务已存在：" + ConnectionUtil.QUEUE_SEND_NAME10);
        }
...
//            channel.queueDeclare(ConnectionUtil.QUEUE_SEND_NAME10, false, false, true, null);
//            String message1 = "";
...
//                message1 = "start job begin" + QUEUE_SEND_NAME10 + args[0];
            }
            //exchange -- 交换机名称
            // routingKey -- 路由关键字
            ...
            channel.basicPublish("", ConnectionUtil.QUEUE_SEND_NAME10, null, object1.toJSONString().getBytes());
            logger.info("发送的消息，通道是：" + ConnectionUtil.QUEUE_SEND_NAME10 + " sends：" + object1.toJSONString());
            // 声明接收队列（创建）队列
            channel.queueDeclare(ConnectionUtil.QUEUE_REV_NAME10, false, false, true, null);
            // 定义队列的消费者
            QueueingConsumer consumer = new QueueingConsumer(channel);
            // 监听队列
            channel.basicConsume(ConnectionUtil.QUEUE_REV_NAME10, true, consumer);
            // 获取消息
            QueueingConsumer.Delivery delivery = consumer.nextDelivery();
            String message2 = new String(delivery.getBody());
            logger.info("接收的消息，通道是：" + ConnectionUtil.QUEUE_REV_NAME10 + " Received：" + message2);
...
//            if (message2.equals(""))
//            JobStep upJobStep = new JobStep();
//            upJobStep.setJobname(QUEUE_REV_NAME10);
...
//            logger.info("更新完成 JobStep: " + QUEUE_REV_NAME10 + " 条数：" + update);
